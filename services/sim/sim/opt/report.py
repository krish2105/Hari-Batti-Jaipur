...
def build() -> str:
    ...
    day = config.SIM_DIR / "reports" / "calibration_day.json"
    cal_share = f"{json.loads(day.read_text())['calibration_share']:.0%}" if day.exists() else "about 47%"
    ...
    L = [
        ...
        (
            ...
            f"(full-day corridor GEH<5 {cal_share}, see services/sim/reports/calibration.md), with ASSUMED lanes and timings — "
    ...
    if c.get("isolated"):
        ...
        for j, rs in c["isolated"].items():
            ...
            L.append(
                f"| {j} | " + " | ".join(_m(by[k]["travelTimeS"]) if k in by else "—" for k in ctrls) + " |"
            )
        L.append("")
    if c.get("distilled"):
        L += [
        ...
        for d in c["distilled"]:
            ...
            for j, p in d["junctions"].items():
                stages = "; ".join(
                    f"{lab} {g}" for lab, (_s, g) in zip(p["labels"], p["stages"], strict=True)
                )
    ...
    L += [
        ...
        (
            "- The corridor PPO agent had only 100,000 training steps (about 85 min on a laptop CPU) and did worse than every "
            "fixed plan; it needs far more training before its ranking means anything. The single-junction agent (150,000 "
            "steps) is the one that learned something useful."
        ),
        (
            "- The green wave does not help here: with the corridor oversaturated, queues from one junction block the next, so a "
            "band of green cannot be used. It becomes useful once capacity problems are fixed."
        ),
        "",
        (
