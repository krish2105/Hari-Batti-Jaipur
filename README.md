...
| `docs/` | [Overview](docs/00-overview.md) · [website](docs/01-website.md) · [dashboard](docs/02-dashboard.md) · [mobile](docs/03-mobile.md) · [runbook](docs/04-runbook.md) · [data findings](docs/05-data-analysis.md) · [data connectors](docs/data-connectors.md) · [pilot operations](docs/pilot-operations.md) · [scaling](docs/scaling.md) · [hosting in India](docs/deploy-india.md) |
...
| India hosting (P8 W18) | 🟡 Ready to deploy, not live (needs the owner's provider and budget) | Production stack in `infra/`: API and dashboard images (non-root; API refuses to start with development settings), Caddy with automatic HTTPS, private Postgres/Redis, daily AES-256 encrypted backups kept 30 days, restore script; restore drill done (11 s, all row counts matched); deploy-on-tag workflow to GitHub's registry, server step off until secrets exist. Options compared: E2E Networks, AWS Mumbai, Azure Pune (≈ ₹3,000–5,000/month, all MeitY empanelled). [Guide](docs/deploy-india.md) |
